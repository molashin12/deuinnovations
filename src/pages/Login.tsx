...
import { loginAdmin, getCurrentUser, isFirebaseConfigured } from "../firebase";
import { 
  Lock, 
  Mail, 
  Eye, 
...
export const Login: React.FC = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (err.message === "auth/user-not-found" || err.code === "auth/user-not-found") {
      ...
      <div className="w-full max-w-md z-10">
        ...
        <div className="deu-contact-panel p-8 rounded-xl shadow-xl border border-[var(--deu-line)] bg-[var(--deu-panel-strong)]/40 backdrop-blur-md flex flex-col gap-6">
          ...
          {!isFirebaseConfigured() && (
            <div className="p-3.5 rounded bg-[var(--deu-primary-soft)] border border-[var(--deu-primary)]/30 text-[var(--deu-ink-2)] text-[11px] leading-relaxed flex gap-2">
              <AlertTriangle className="w-5 h-5 text-[var(--deu-primary)] shrink-0" />
              <div>
                <strong>Sandbox Mode:</strong> Firebase is unconfigured. Default admin:<br />
                <span className="font-mono text-[var(--deu-primary)]">admin@deuinnovation.com</span> / <span className="font-mono text-[var(--deu-primary)]">admin123</span>
              </div>
            </div>
          )}
