...
export const getBriefs = async (): Promise<ProjectBrief[]> => {
  ...
  try {
    ...
    // Set up a query timeout to prevent hanging if connection fails
    ...
    // Return empty array on failure/timeout to prevent admin dashboard from breaking
    return [];
  }
};
...
// Default portfolio projects fallback data (also used for seeding Firestore)
export const localFallbacks: PortfolioProject[] = [
  {
    ...
    detailsDe: "Diese Logistikanwendung verbindet Altsysteme (ERP) mit einer modernen Echtzeit-Tracking-Schnittstelle. Durch den Einsatz von WebSockets für sofortige Statusänderungen und maßgeschneiderten Kartenvisualisierungen löst das Leitsystem das visuelle Rauschen auf, das man normalerweise in Transportmanagement-Tools findet. Es führt eine intelligente Warteschlange ein, die lokale heuristische Algorithmen nutzt, um dringende Frachtrouten zu priorisieren. Das Ergebnis is eine einheitliche App, die auf Desktop und Tablets unter Lagerbedingungen sauber läuft.",
...
// Helper to seed Firestore database with default projects
export const seedPortfolioDatabase = async (): Promise<void> => {
  if (!isFirebaseConfigured()) {
    console.warn("Firebase not configured. Simulating database seed.");
    // Simulate database seed in local storage
    return;
  }
  for (const proj of localFallbacks) {
    try {
...
export const getPortfolioProjects = async (): Promise<PortfolioProject[]> => {
  ...
  try {
    ...
    // Set up a query timeout to prevent hanging if connection fails
    const fetchPromise = getDocs(projectsCol);
    const timeoutPromise = new Promise<never>((_, reject) => 
      setTimeout(() => reject(new Error("Timeout")), 2500)
    );
